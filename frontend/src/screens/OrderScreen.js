...
export default function OrderScreen(props) {
    ...
    useEffect(() => {
        ...
        const addPayPalScript = async () => {
            ...
            document.body.appendChild(script); //this script will be added at last in the html document
        };

        if (!order || successPay || successDeliver || (order && order._id !== orderId)) { //if order not yet loaded
            dispatch({ type: ORDER_PAY_RESET });
            dispatch({ type: ORDER_DELIVER_RESET });
            dispatch(detailsOrder(orderId));
        } else {
            if (!order.isPaid) { //if order is not yet paid
                if (!window.paypal) { //if paypal is not yet loaded
                    addPayPalScript();
                }
                else {
                    setSdkReady(true);
                                ...
                                <ul>
                                    ...
                                    {!order.isPaid && (
                                        <li>
                                            {!sdkReady ? (<LoadingBox></LoadingBox>) :
                                                (
                                                    <>
                                                        {errorPay && (<MessageBox varient="danger">{error}</MessageBox>)}
                                    ...
                                    {userInfo.isAdmin && order.isPaid && !order.isDelivered && (
                                        <li>
                                            ...
                                            {errorDeliver && (<MessageBox varient="danger">{errorDeliver}</MessageBox>)}
